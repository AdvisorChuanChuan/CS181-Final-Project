...
class World:
    def __init__(self, _map=Mymap()):
        ...
        csv_data.to_csv('./log_lca/iteration.csv', mode="w", index=True)
    ...
    @staticmethod
    def decoder():
        with open("./log_lca/policy1.json", "r") as f:
            b = json.loads(f.read())
            re = util.Counter()
    ...
    def log_iteration(self, score, success, fail):
        ...
        csv_data.to_csv('./log_lca/iteration.csv', mode="a", header=False, index=True)

        if self.best_score < score:
            self.best_score = score
            with open('./log_lca/policy.json', 'w') as f:
